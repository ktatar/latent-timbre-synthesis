...
train_buf = batch_size * config['training'].getint('train_buf_multiplier')
...
class Encoder(layers.Layer):
  ...
  def __init__(self,
    ...
    self.reshape = tf.keras.layers.Reshape(input_shape=(self.original_dim,),target_shape=(self.cqt_bins, 8, 1))
    self.conv2D1 = tf.keras.layers.Conv2D(input_shape=(self.cqt_bins, 8, 1),
              filters=32, kernel_size=3, strides=(2, 2), activation='relu')
    self.conv2D2 = tf.keras.layers.Conv2D(input_shape=(self.cqt_bins/2, 8, 1),
...
class Decoder(layers.Layer):
  ...
  def __init__(self,
    ...
    self.cqt_bins = cqt_bins
    self.latent_dim = latent_dim
    self.denseproj = tf.keras.layers.Dense(input_shape=(latent_dim,), units=int(self.cqt_bins/4)*int(8/4)*32, activation=tf.nn.relu)
    self.reshape = tf.keras.layers.Reshape(input_shape=(int(self.cqt_bins/4)*int(8/4)*32,), target_shape=(int(self.cqt_bins/4), int(8/4), 32))
    self.conv2DT1 = tf.keras.layers.Conv2DTranspose(input_shape=(int(self.cqt_bins/4), int(8/4), 32),
        filters=64,
        kernel_size=3,
        strides=(2, 2),
        padding="SAME",
        activation='relu')
    self.conv2DT2 = tf.keras.layers.Conv2DTranspose(input_shape=(int(self.cqt_bins/2), int(8/2), 64),
        filters=32,
        kernel_size=3,
        strides=(2, 2),
        padding="SAME",
        activation='relu')
    # No activation
    self.conv2DT3 = tf.keras.layers.Conv2DTranspose(input_shape=(int(self.cqt_bins), 8, 32),
        filters=1, kernel_size=3, strides=(1, 1), padding="SAME")
    self.reshape_out = tf.keras.layers.Reshape(target_shape=(self.cqt_bins*8,))

  def call(self, inputs):
    x = self.denseproj(inputs)
    x = self.reshape(x)
    x = self.conv2DT1(x)
    x = self.conv2DT2(x)
    x = self.conv2DT3(x)
    return self.reshape_out(x)
...
class VariationalAutoEncoder(tf.keras.Model):
  ...
  def __init__(self,
    ...
    self.latent_dim = latent_dim
    self.encoder = Encoder(latent_dim=latent_dim, original_dim=original_dim, cqt_bins=self.cqt_bins)
    self.decoder = Decoder(cqt_bins=self.cqt_bins, latent_dim=latent_dim)
